...
function documentKindFromPath(config, filePath) {
    ...
    if (normalizedPath.includes('/docs/validation/'))
    ...
    if (relativeProjectPath.startsWith('ProductHarness/Validation/'))
        return 'harness-validation';
    if (relativeProjectPath.startsWith('ProductHarness/Decisions/'))
        return 'harness-decision';
    if (relativeProjectPath.startsWith('ProductHarness/'))
        return 'harness-product';
...
function collectRecallFilePaths(config, repoRoot) {
    const candidates = [
        ...
        ...recentMarkdownFiles(node_path_1.default.join(config.project_root, 'Plans'), MAX_MARKDOWN_FILES_PER_DIR, true),
        ...recentMarkdownFiles(node_path_1.default.join(config.project_root, 'ProductHarness'), MAX_MARKDOWN_FILES_PER_DIR, true),
    ...
    if (repoRoot) {
        candidates.push(...recentMarkdownFiles(node_path_1.default.join(repoRoot, 'docs', 'superpowers', 'plans'), MAX_MARKDOWN_FILES_PER_DIR, true));
        candidates.push(...recentMarkdownFiles(node_path_1.default.join(repoRoot, 'docs', 'product'), MAX_MARKDOWN_FILES_PER_DIR, true));
        candidates.push(...recentMarkdownFiles(node_path_1.default.join(repoRoot, 'docs', 'stories'), MAX_MARKDOWN_FILES_PER_DIR, true));
        candidates.push(...recentMarkdownFiles(node_path_1.default.join(repoRoot, 'docs', 'validation'), MAX_MARKDOWN_FILES_PER_DIR, true));
        candidates.push(...recentMarkdownFiles(node_path_1.default.join(repoRoot, 'docs', 'decisions'), MAX_MARKDOWN_FILES_PER_DIR, true));
    }
...
function buildRecallIndex(config, repoRoot) {
