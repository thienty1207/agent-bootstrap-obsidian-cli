...
- Coding style and minimal-edit behavior: use `karpathy-coding-principles/`.
- Public product API design with no model/provider boundary.
- System-wide architecture, ADRs, or service topology; use the `architect` subagent when delegation is useful.
- Auth, input hardening, deployment, database, or cloud-only work unless it directly affects the provider integration boundary.
- Provider-specific SDK signatures, current limits, or latest model names without checking current official docs first.
