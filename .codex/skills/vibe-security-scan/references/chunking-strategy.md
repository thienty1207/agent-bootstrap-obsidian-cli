...
- Mỗi chunk có **scope rõ ràng**: 1 phần logic của repo (không phải 1 file lẻ)
- Mỗi chunk có **đủ context** để reasoning (không bị cắt mid-function)
- **Cân bằng tải**: chunks tương đương kích cỡ
- **Không quá nhiều chunks**: target 3-10 chunks (quá nhiều chunk làm aggregation chậm và nhiễu)
...
3. **Detected primary language ≠ folder language** → ưu tiên giữ chunk theo lang để chunk có thể apply đúng overlay
...
KHÔNG quá 15 chunks. Quá nhiều chunk làm scan chậm hơn và tăng context aggregate.
...
Mỗi chunk được xử lý như 1 task tuần tự. Ghi findings theo schema trong `references/output-format.md`.
