...
Inline scan workflow cho repo nhỏ-vừa (≤20 main-lang files VÀ ≤30 total VÀ ≤14 ngày). Main agent tự chạy hết, không process chunk sequentially.
...
- Nếu phát hiện 1 issue mà không có rule khớp 100%, MAP về rule canonical gần nhất trong `SKILL.md` và note lý do trong `issue`.
